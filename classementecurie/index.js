...
for (const pilote of data) {
    ...
    nomCell.innerText = pilote.nomEcurie;
    ...
    for (const point of pointsParGP) {
        // Création de la balise <td> pour chaque point par grand prix
        const tdPointParGP = document.createElement('td');
        tdPointParGP.style.padding = '2px'; // espacement entre les points
        tdPointParGP.style.fontSize = '14px'; // taille de la police

        // Séparer le point en deux parties (point et place)
        ...
        tdPointParGP.appendChild(span); // ajouter le point par grand prix à la cellule

        tdPointsParGP.appendChild(tdPointParGP); // ajouter la cellule au tableau

        // Ajouter l'image à la cellule des images des pays
        tdImagesPays.appendChild(img);
    }

    // Ajouter la cellule des images des pays à la ligne
    tr.insertBefore(tdImagesPays, tr.cells[3]); // Insérer la cellule avant la colonne des points
}
